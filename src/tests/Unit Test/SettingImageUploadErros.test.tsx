...
describe('Settings Image Upload', () => {
  ...
  test('displays an error message for image with wrong format', async () => {
    ...
    const fileInput = screen.getByTestId('file-input');
    const wrongFormatImage = new File(['(⌐□_□)'], 'WrongFormat.webp', {
      type: 'image/webp',
    });
    Object.defineProperty(Image.prototype, 'width', { value: 799 });
    Object.defineProperty(Image.prototype, 'height', { value: 799 });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          /Unsupported file format. Please upload a JPG, PNG, GIF, or SVG./i,
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  test('displays an error message for image with wrong resolution', async () => {
    ...
    const fileInput = screen.getByTestId('file-input');
    const wrongResImage = new File(['(⌐□_□)'], 'WrongRes.png', {
      type: 'image/png',
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          /Image resolution is too high. Maximum allowed is 800x800px./i,
        ),
      ).toBeInTheDocument();
    });
  });
  ...
  test('displays an error message for image with both wrong format and resolution', async () => {
    ...
    const fileInput = screen.getByTestId('file-input');
    const wrongResAndFormatImage = new File(
      ['(⌐□_□)'],
      'WrongResAndFormat.webp',
      { type: 'image/webp' },
    );
    Object.defineProperty(Image.prototype, 'width', { value: 801 });
    Object.defineProperty(Image.prototype, 'height', { value: 801 });
    ...
    fireEvent.change(fileInput, {
      target: { files: [wrongResAndFormatImage] },
    });

    await waitFor(() => {
      expect(
        screen.getByText(
          /Unsupported file format and resolution too high. Please upload a JPG, PNG, GIF, or SVG with a maximum resolution of 800x800px./i,
        ),
      ).toBeInTheDocument();
    });
  });
